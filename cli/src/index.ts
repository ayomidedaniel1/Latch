...
async function forwardWebhook(event: WebhookEvent, forwardTo: string) {
  ...
  try {
    ...
    console.log(
      `[${DIM}${timestamp}${RESET}] [${BLUE}Event ${event.id.slice(0, 8)}${RESET}] Forwarded to ${BOLD}${forwardTo}${RESET} -> ${statusColor}${response.status} ${response.statusText}${RESET} (${duration}ms)`
    );
    ...
    console.error(
      `[${DIM}${timestamp}${RESET}] [${BLUE}Event ${event.id.slice(0, 8)}${RESET}] ${RED}Failed to forward to ${forwardTo} -> ${errorMessage}${RESET} (${duration}ms)`
    );
  }
}

async function connectSSE(projectId: string, token: string, apiUrl: string, forwardTo: string) {
      ...
      for await (const chunk of stream) {
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith('id:')) {
            ...
            if (dataStr === 'connected') continue;

            try {
              const event: WebhookEvent = JSON.parse(dataStr);
