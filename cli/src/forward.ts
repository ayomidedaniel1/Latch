...
export async function forwardWebhook(event: WebhookEvent, forwardTo: string): Promise<{ status: number; duration: number }> {
  ...
  const forwardHeaders: Record<string, string> = { ...event.headers };
  ...
  try {
    ...
    console.log(
      `[${DIM}${timestamp}${RESET}] [${BLUE}Event ${eventId.slice(0, 8)}${RESET}] Forwarded to ${BOLD}${forwardTo}${RESET} -> ${statusColor}${response.status} ${response.statusText}${RESET} (${duration}ms)`
    );
    ...
    console.error(
      `[${DIM}${timestamp}${RESET}] [${BLUE}Event ${eventId.slice(0, 8)}${RESET}] ${RED}Failed to forward to ${forwardTo} -> ${errorMessage}${RESET} (${duration}ms)`
    );
  }
