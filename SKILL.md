...
export async function GET(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // First connection (no Last-Event-ID): seed the feed with the most
      // recent events so the dashboard isn't empty on load, then continue
      // tailing forward from there.
      if (!cursorTimestamp) {
...
export function EventFeed({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      ...
      setEvents(prev => {
        if (prev.some(p => p.id === event.id)) return prev; // de-dupe on reconnect overlap
        return [event, ...prev].slice(0, 200); // cap at 200 in memory
      });
    };
